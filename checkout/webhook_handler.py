...
class StripeWH_Handler:
    ...
    def handle_payment_intent_succeeded(self, event):
        ...
        """
        logging.info(f"Processing webhook event: {event.data.object}")
        intent = event.data.object
        pid = intent.id
        bag = intent.metadata.bag
        save_info = intent.metadata.save_info

        # Retrieve the Charge object using intent.latest_charge
        try:
            stripe_charge = stripe.Charge.retrieve(intent.latest_charge)
        except Exception as e:
            logging.error(f"Error retrieving charge: {e}")
            return HttpResponse(
                content=f"Webhook received: {event['type']} |\
                      ERROR: Could not retrieve charge", status=500)

        # Use billing details from the charge
        billing_details = stripe_charge.billing_details
        shipping_details = intent.shipping
        # Use the amount from the charge
